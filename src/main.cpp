...
std::vector<std::vector<int>> findNeighbours(const std::vector<Particle>& particles) {
    int n = particles.size();
    
    std::vector<std::vector<int>> neighbours(n);

    for (int i = 0; i < n; i++) neighbours[i].reserve(64);
...
} else if constexpr (RHO_VAR == 2) {
        // ! var 2 Shepard normalized
...
#ifdef TEST
void test_compute_density_pressure() {
    // init particles
    std::vector<Particle> particles;
    for (float x = 1.0f + DX / 2.0f; x < 2.0f; x += DX) {
        for (float y = 1.0f + DX / 2; y < 2.0f; y += DX) {
            Particle p;
            p.x = x; p.y = y;
    ...
    computeDensityPressure(particles, neighbours);

    for (int i = 0; i < particles.size(); i++) {
        std::cout << "particle " << i << ": density " << particles[i].rho << " pressure: " << particles[i].pressure << "\n"; 
    }
}

void test_compute_stress() {
    // init particles
    std::vector<Particle> particles;
    for (float x = 1.0f + DX / 2.0f; x < 2.0f; x += DX) {
        for (float y = 1.0f + DX / 2; y < 2.0f; y += DX) {
    ...
    for (int i = 0; i < particles.size(); i += 100) {
        std::cout << "particle " << i << ": pxx " << particles[i].pxx << " pxy " << particles[i].pxy << " pyy " << particles[i].pyy << "\n"; 
    }
}

void test_compute_acceleration() {
    // init particles
    std::vector<Particle> particles;
    for (float x = 1.0f + DX / 2.0f; x < 2.0f; x += DX) {
        for (float y = 1.0f + DX / 2; y < 2.0f; y += DX) {
    ...
    for (int i = 0; i < particles.size(); i += 100) {
        std::cout << "particle " << i << ": ax " << particles[i].ax << " ay " << particles[i].ay << "\n"; 
    }
}
...
void test_one_step() {
    ...
    for (int i = 0; i < particles.size(); i += 37) {
